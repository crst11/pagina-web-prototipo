/*
    LocalShop - inicializacion idempotente para Docker.

    Este script conserva datos existentes. Crea la base TiendaMicroempresas,
    agrega tablas faltantes y aplica columnas que necesita el backend actual:
    empresas, clientes, sesiones, productos, pedidos, metodo de pago y
    archivado logico de productos.
*/
...
BEGIN
    ...
    ADD IsArchived BIT NOT NULL CONSTRAINT DF_Products_IsArchived DEFAULT 0;
END;
GO
...
BEGIN
    ...
    ADD PaymentMethod NVARCHAR(80) NOT NULL
        CONSTRAINT DF_Orders_PaymentMethod DEFAULT N'No especificado';
END;
GO

IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_Orders_Businesses')
BEGIN
    ALTER TABLE dbo.Orders DROP CONSTRAINT FK_Orders_Businesses;
END;
GO

ALTER TABLE dbo.Orders ALTER COLUMN BusinessId INT NULL;
GO

IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_Orders_Businesses')
BEGIN
    ALTER TABLE dbo.Orders
    ADD CONSTRAINT FK_Orders_Businesses FOREIGN KEY (BusinessId) REFERENCES dbo.Businesses(BusinessId);
END;
GO
